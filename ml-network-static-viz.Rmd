...
source("~/Dropbox/07_visualization/network/02.code/static-rmarkdown-code.R")
knitr::opts_chunk$set(echo=TRUE, message=FALSE, warning=FALSE)
```


## 1. 네트워크 데이터 시각화 {#network-static-viz}
...
### 1.1. `igraph` 주요 설정 매개변수 {#network-static-viz-parameters}
...
### 1.2. 노드와 엣지에 매개변수 반영  {#network-static-viz-parameters}
...
## 2. 네트워크 배치(Layout) {#network-static-viz-layout}
...
``` {r network-layout-setting, warning=FALSE}
# 사전에 노드좌표를 설정
...
### 2.1. 네트워크 배치 알고리즘 {#network-static-viz-layout-algorithm}
...
``` {r network-layout-algorithm, warning=FALSE}
...
``` {r network-layout-algorithms, warning=FALSE}
# igraph 내장 배치 알고리즘
...
## 3. 네트워크 하이라이트(Highlight) {#network-static-viz-highlight}
...
``` {r network-why-highlight, warning=FALSE}
plot(net)
```
...
``` {r network-remove-edge, warning=FALSE}
#------------------------------------------------------------------------
# 01.01. 엣지 제거
...
### 3.1. 엣지 제거 또다른 방법  {#network-static-viz-remove-edge}

엣지를 제거하는 다른 방법으로 하이퍼링크(hyperlink) 네트워크와 
언급(mention) 네트워크를 별도로 설정하여 이를 네트워크 배치 알고리즘에 넣어
시각화를 하는 것도 가능한 방법이다.

``` {r network-remove-edge-method, warning=FALSE}
#------------------------------------------------------------------------
# 02. 원배치 : 하이퍼링크, 언급
...
### 3.2. 특정 노드 혹은 엣지 하이라이트 강조 {#network-static-viz-node-highlight}

`shortest.paths` 함수는 해당 네트워크 노드 간에 최단경로 행렬정보를 처리하여 반환한다.
예를 들어, NYT와 다른 언론기관간에 거리를 시각화보자.

``` {r network-hl-shortest-path, warning=FALSE}
#------------------------------------------------------------------------
# 01. 최단거리
...
``` {r network-hl-adjacency, warning=FALSE}
#------------------------------------------------------------------------
# 02. WSJ 인접 노드
...
``` {r network-hl-mark, warning=FALSE}
#------------------------------------------------------------------------
# 03. 표식 Mark
...
``` {r network-hl-path, warning=FALSE}
#------------------------------------------------------------------------
# 04. 경로 표시
